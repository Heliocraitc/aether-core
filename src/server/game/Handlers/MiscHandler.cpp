...
void WorldSession::HandleRepopRequestOpcode(WorldPacket& recvData)
{
    ...
    */
    ...
    if(player->FindNearestCreature(90000, 30)) //Custom edits
      {
        Creature* overseer = player->FindNearestCreature(90000, 30);
        player->TeleportTo(overseer->GetMapId(), overseer->GetPositionX(), overseer->GetPositionY(), overseer->GetPositionZ(), overseer->GetOrientation() + 180);
        overseer->Respawn();
      }
      else
      {
        player->SummonCreature(90000, player->GetPositionX(), player->GetPositionY(), player->GetPositionZ(), player->GetOrientation(), TEMPSUMMON_TIMED_DESPAWN, 300000);
      }
    //GetPlayer()->RepopAtGraveyard(); //Custom edit, stop the player from reappearing at a graveyard
...
void WorldSession::HandleWhoOpcode(WorldPacket& recvData)
{
    ...
    */
        ...
        uint32 pzoneid = AREA_FLAG_UNUSED1;
    ...
    */
        ...
        std::string gname = sGuildMgr->GetGuildNameById(target->GetGuildId()); //Guild name string that appears in Who List ~ Heliocratic
        std::wstring wgname;
...
void WorldSession::HandleResurrectResponseOpcode(WorldPacket& recvData)
{
    ...
    */
    if (!GetPlayer()->isResurrectRequestedBy(guid))
        return;
    //HELIOCRATIC: Not letting the accept data go through, to ressurect the player.
